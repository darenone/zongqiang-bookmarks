...
css雪碧图又叫css精灵图或图片整合技术，常用的浏览器优化的一个方法，把项目中的小图标都放到一张png图片上，然后根据`background-position`这个属性，来控制位置，使用这些小图标，目的是，在项目上线以后，浏览器只需要请求一次，拿到这个png图片就行，而不是一个小图标请求一次服务器，大大减少了服务器资源的消耗

## 2. 使用rem

rem（font size of the root element）相对于根元素字体大小的单位

假如手机宽度是375px，把它16等分，`375/16=23.4375`，每一份就是23.4375，咱们把这一份如果称之为1rem，那么也就可以理解为`1rem=23.4375`

假如手机宽度是1080px，我把它16等分，`1080/16=67.5`，每一份就是67.5，咱们把这一份如果称之为1rem，那么也就可以理解为`1rem=67.5`
...
具体是等分为16份还是等分为18分，可以根据你们公司产品经理的要求来做
...
BFC 即 Block Formatting Contexts (块级格式化上下文)，它属于上述定位方案的普通流，可以通过阅读这篇文章便于理解[10 分钟理解 BFC 原理](https://zhuanlan.zhihu.com/p/25321647)
