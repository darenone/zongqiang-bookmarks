...
└── update.sh
```
备注<sup>Tip</sup>：如果我的目录不符合你的需求，可以根据自己的要求，按照我这个目录结构改哦
...
找到`vuePress\docs\package.json`的`package.json`文件，配置如下：
```
...
找到`vuePress\docs\.vuepress\config.js`的`config.js`文件，找到base字段，如果想要发布到如下图链接
...
base: '/zongqiang-bookmarks/', // 部署到GitHub相关配置
```
找到`vuePress`根目录，新建`deploy.sh`脚本文件
```git
#!/usr/bin/env sh
